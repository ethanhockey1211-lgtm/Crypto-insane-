...
import { americanDecimal, boardProjections, centralDate, entryAnalysis, historyEstimate, noVig, numeric, percent, PICK_STORAGE_KEY, rankBoard, readEntries, riskLabel, SPORTS, stalePick, STATS, SPORT_STATS, type Estimate, type Pick, type PropLine, type PicksBoard, type SavedEntry, type Side } from "@/lib/prizepicks";
...
function EstimateView({ pick, now }: { pick: Pick; now: number }) {
  ...
  return <div className="mt-3">
    ...
    <p className="text-ink-2 mt-1">{(pick.method === "history" || pick.method === "nhl-history") ? "History-based estimate" : "Market-implied estimate"}{pick.estimate.conditional ? " · conditional on no tie" : " · strict hit chance"}</p>
    ...
    <p className="text-ink-2 mt-2">{(pick.method === "history" || pick.method === "nhl-history") ? "Historical interval" : "Book range"}: {percent(pick.estimate.low)}–{percent(pick.estimate.high)}</p>
  </div>;
}

function CustomPick({ onAdd, selected }: { onAdd: (pick: Pick) => void; selected: PropLine | null }) {
  const [player, setPlayer] = useState(""); const [stat, setStat] = useState("player_shots_on_goal");
  const [line, setLine] = useState(""); const [side, setSide] = useState<Side>("More");
  const [sport, setSport] = useState("icehockey_nhl"); const [game, setGame] = useState(""); const [startsAt, setStartsAt] = useState("");
  ...
  useEffect(() => {
    if (!selected) return;
    setPlayer(selected.player); setSport(selected.sport); setStat(selected.stat); setLine(String(selected.line)); setGame(selected.matchup);
    const local = new Date(Date.parse(selected.startsAt) - new Date(selected.startsAt).getTimezoneOffset() * 60000);
    setStartsAt(local.toISOString().slice(0, 16)); setMethod("history"); setScores(selected.history?.map(s => s.value).join(", ") ?? "");
  ...
  return <section className="panel rounded-lg p-4 sm:p-5" id="custom-pick">
    ...
    <form onSubmit={analyze} onChange={invalidate} className="grid gap-4">
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <label>League<select className="field mt-1" value={sport} onChange={e => { setSport(e.target.value); setStat(SPORT_STATS[e.target.value][0]); }}>{Object.entries(SPORTS).map(([key, name]) => <option value={key} key={key}>{name}</option>)}</select></label>
        <label>Stat<select className="field mt-1" value={stat} onChange={e => setStat(e.target.value)}>{SPORT_STATS[sport].map(key => <option value={key} key={key}>{STATS[key]}</option>)}</select></label>
...
export default function PrizePicksWorkspace() {
  const [now, setNow] = useState(Date.now()); const [sport, setSport] = useState("icehockey_nhl");
  const [selected, setSelected] = useState<PropLine | null>(null);
  ...
  const projections = boardProjections(board, now);
  const unranked = projections.filter(p => !ranked.some(r => r.eventId === p.eventId && r.player === p.player && r.stat === p.stat && r.line === p.line));
  function analyzeLine(p: PropLine) { setSelected({ ...p }); document.getElementById("custom-pick")?.scrollIntoView({ block: "start", behavior: "smooth" }); }
  ...
  return <main className="prizepicks-workspace h-full overflow-y-auto min-w-0 pr-1" data-testid="prizepicks-workspace">
    <header className="panel rounded-lg p-4 sm:p-5 mb-3 flex flex-wrap gap-4 justify-between items-start">
      ...
      <div className="text-ink-2"><p className="num">{centralDate(now)} · Central time</p><p className="mt-2">Standard projections · NHL / NBA / WNBA / NFL / MLB</p></div>
    </header>
    <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_380px] items-start">
      <div className="min-w-0 grid gap-3">
        <section className="panel rounded-lg p-4 sm:p-5" aria-labelledby="daily-picks-heading">
          <div className="flex flex-wrap gap-3 items-center justify-between"><div><h2 className="text-xl font-semibold" id="daily-picks-heading">Recommended picks for today</h2><p className="text-ink-2 mt-1">Current PrizePicks lines, ranked by estimated hit chance where supporting data is available.</p></div><a className="control-button" href="#custom-pick" onClick={e => { e.preventDefault(); document.getElementById("custom-pick")?.scrollIntoView({ block: "start", behavior: "smooth" }); }}>Check my own pick ↓</a></div>
          ...
          <p className="text-ink-2 mt-3">The board loads when you choose a league. Refresh to check for line changes; current-line evidence expires after five minutes.</p>
          {feedMessage && <p role="alert" className="text-warn mt-3">{feedMessage}</p>}
          {board && <p role="status" className="text-ink-2 mt-3">{board.source ?? "PrizePicks feed"} · {board.message} Last scan {new Date(board.asOf).toLocaleTimeString()} · {board.eventsScanned}/{board.eventsAvailable} events.</p>}
          {board?.evidenceMessage && <p className="text-warn mt-2">{board.evidenceMessage}</p>}
          ...
          <div className="grid md:grid-cols-2 gap-3 mt-4" aria-live="polite" aria-busy={loading}>
            {visible.map((pick, i) => <article key={pick.id} className="rounded-lg bg-ground border border-line-strong p-4">
              ...
              <EstimateView pick={pick} now={now} /><p className="text-ink-2 mt-2">{pick.books.join(" · ")} · line checked {Math.max(0, Math.floor((now - Date.parse(pick.updatedAt)) / 60000))}m ago</p>
              <button className="control-button mt-4 w-full" onClick={() => add(pick)}>Add to my entry</button>
            </article>)}
          </div>
          {!loading && !visible.length && <div className="border border-dashed border-line-strong p-5 rounded-lg mt-3"><p className="font-semibold">{query && ranked.length ? "No matching players" : feedMessage ? "Live feed unavailable" : board ? "No supported recommendation right now" : "Loading today’s PrizePicks board"}</p><p className="text-ink-2 mt-2">{board ? "A posted line needs supporting evidence: two exact sportsbook matches, or at least 10 recent official NHL results. No upcoming lines means there is no daily pick to recommend. Demons, Goblins and period props are excluded." : "Live recommendations require a reachable PrizePicks feed. You can still check your own selections below."}</p></div>}
          {unranked.length > 0 && <details className="mt-5 border-t border-line-strong pt-4" open><summary className="font-semibold cursor-pointer">Current PrizePicks board · {unranked.length} lines without an automatic estimate</summary><div className="grid md:grid-cols-2 gap-3 mt-3">{unranked.map(p => <article key={p.projectionId ?? `${p.eventId}:${p.player}:${p.stat}:${p.line}`} className="border border-line-strong rounded-lg p-3"><h3 className="font-semibold">{p.player}</h3><p className="mt-1">{p.line} {STATS[p.stat]} · More / Less</p><p className="text-ink-2 mt-1">{p.matchup} · {new Date(p.startsAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</p><p className="text-ink-2 mt-2">Hit chance unavailable — insufficient evidence.</p><button className="control-button mt-3" onClick={() => analyzeLine(p)}>Analyze this line</button></article>)}</div></details>}
          <a className="text-accent underline inline-block mt-4" href="https://app.prizepicks.com/" target="_blank" rel="noopener noreferrer">Verify lines in PrizePicks ↗</a>
        </section>
        <CustomPick onAdd={add} selected={selected} />
      </div>
