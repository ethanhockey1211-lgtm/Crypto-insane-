...
export function EntryAlerts({ onOpen }: { onOpen: (symbol: string) => void }) {
  ...
  const desktopNotice = useRef<{ notification: Notification; symbols: string[] } | null>(null);
  useEffect(() => {
    try {
    ...
    setLoaded(true);
    return () => { void audio.current?.close(); audio.current = null; desktopNotice.current?.notification.close(); desktopNotice.current = null; };
  }, []);
  useEffect(() => { if (loaded) { try { localStorage.setItem(KEY, JSON.stringify(prefs)); } catch { /* optional */ } } }, [prefs, loaded]);
  useEffect(() => {
    // Restoring an already eligible market must not manufacture a new entry-zone visit.
    tracker.current.pause();
    setNotices(previous => previous.filter(notice => !hiddenMarkets.isHidden(notice.row.symbol)));
    if (desktopNotice.current?.symbols.some(symbol => hiddenMarkets.isHidden(symbol))) {
      desktopNotice.current.notification.close(); desktopNotice.current = null;
    }
  }, [visibility.symbols]);
  useEffect(() => {
    const t = tracker.current;
    if (!prefs.enabled) { t.pause(); return; }
    const fresh = t.update(store.getOrder().flatMap(s => { const r = store.getRow(s); return r && !hiddenMarkets.isHidden(s) ? [r] : []; }), hub === "connected" && feed?.live === true, cycle.at, Date.now());
    ...
    if (prefs.desktop && typeof Notification !== "undefined" && Notification.permission === "granted") {
      try {
        ...
        desktopNotice.current = { notification, symbols: fresh.map(notice => notice.row.symbol) };
        notification.onclick = () => { if (!hiddenMarkets.isHidden(first.symbol)) { window.focus(); onOpen(first.symbol); } notification.close(); };
      } catch { setMessage("Desktop notifications are unavailable here; in-page alerts remain active."); }
    }
  }, [cycle, feed, hub, prefs, onOpen, visibility.symbols]);

  const visibleNotices = notices.filter(notice => !hiddenMarkets.isHidden(notice.row.symbol));
  ...
  return <details className="entry-alerts shrink-0 border border-line rounded-lg bg-navy px-3 py-2 text-[12px]">
    <summary className="cursor-pointer flex items-center gap-2 list-none">
      ...
      <span className="text-ink-3 truncate">{visibleNotices.length ? `${visibleNotices[0].row.symbol.replace("-", "/")} · latest zone event` : "Sound + desktop notifications"}</span>
