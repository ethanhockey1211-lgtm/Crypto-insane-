...
function Cell({ label, children, className }: { label: string; children: React.ReactNode; className?: string }) {
  return (
    <div className={`flex flex-col justify-center px-3 border-r border-line whitespace-nowrap ${className ?? ""}`}>
...
function Asset({ a, symbol }: { a: AssetState | null; symbol: string }) {
  ...
  return (
    <>
      <Cell label={symbol.replace("-USD", "")}>{price ? fmtPrice(price) : "—"}</Cell>
      <Cell label="1h"><span className={signClass(a?.r1h)}>{fmtPct(a?.r1h)}</span></Cell>
      <Cell label="24h"><span className={signClass(a?.r24h)}>{fmtPct(a?.r24h)}</span></Cell>
    </>
  );
}
...
export function MarketHeader() {
  ...
  return (
    <header className="glass flex items-stretch h-[52px] overflow-x-auto">
      <div className="flex flex-col justify-center px-3 border-r border-line whitespace-nowrap">
      ...
      <Cell label="BTC vs VWAP">{market?.btc ? (market.btc.aboveVwap ? "above" : "below") : "—"}{market?.btc?.vwapSigma != null ? ` ${market.btc.vwapSigma.toFixed(1)}σ` : ""}</Cell>
      ...
      <Cell label="Up on hour">{market ? `${Math.round(market.breadthPositive1h * 100)}%` : "—"}</Cell>
      <Cell label="Volume trend">{market ? `${fmtX(market.medianRelVolume, 2)} median` : "—"}</Cell>
      <Cell label="BTC dom." className="text-ink-3">n/a</Cell>
      <div className="flex flex-col justify-center px-3 border-r border-line whitespace-nowrap">
