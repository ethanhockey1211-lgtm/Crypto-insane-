...
public class PrizePicksFeedTests
{
    ...
    private static string Odds(string timestamp = "2026-09-11T14:59:00Z", string stat = "player_points") => $$"""
        ...
        """;
    private static PrizePicksFeed Feed(Handler handler, Clock? clock = null, PrizePicksOptions? options = null) => new(new HttpClient(handler), Options.Create(options ?? new PrizePicksOptions { ApiKey = "fixture-provider-key" }), clock ?? new Clock());

    [Fact]
    public async Task Requires_only_provider_configuration_and_a_valid_sport_before_spending_quota()
    {
        var handler = new Handler();
        using var missing = Feed(handler, options: new());
        Assert.False(missing.Configured);
        Assert.Equal(503, (await missing.ScanAsync("basketball_nba", default)).StatusCode);
        using var configured = Feed(handler);
        Assert.True(configured.Configured);
        Assert.Equal(400, (await configured.ScanAsync("../../unknown", default)).StatusCode);
        Assert.Empty(handler.Requests);
    }
    [Fact]
    public async Task Fetches_today_then_event_odds_and_shares_cache_without_an_access_code()
    {
        var handler = new Handler(); var clock = new Clock(); using var feed = Feed(handler, clock);
        var response = await feed.ScanAsync("basketball_nba", default);
        ...
        await feed.ScanAsync("basketball_nba", default);
        Assert.Equal(2, handler.Requests.Count);
        clock.At = Now.AddMinutes(3); await feed.ScanAsync("basketball_nba", default);
        Assert.Equal(4, handler.Requests.Count);
        var body = JsonSerializer.Serialize(response.Body);
        Assert.DoesNotContain("fixture-provider-key", body);
    }
    [Fact]
    public async Task Http_board_is_available_without_credentials_and_keeps_the_provider_key_private()
    {
        var handler = new Handler(); using var feed = Feed(handler);
        var builder = WebApplication.CreateBuilder();
        builder.WebHost.UseTestServer();
        builder.Services.AddSingleton(feed);
        builder.Services.AddRateLimiter(o => o.AddFixedWindowLimiter("api", limit =>
        {
            limit.PermitLimit = 10; limit.Window = TimeSpan.FromMinutes(1);
        }));
        await using var app = builder.Build();
        app.UseRateLimiter(); app.MapPrizePicks(); await app.StartAsync();
        using var client = app.GetTestClient();
        var status = await client.GetFromJsonAsync<JsonElement>("/api/prizepicks/status");
        Assert.True(status.GetProperty("configured").GetBoolean());
        using var response = await client.GetAsync("/api/prizepicks/board?sport=basketball_nba");
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var body = await response.Content.ReadAsStringAsync();
        Assert.Contains("Player A", body);
        Assert.DoesNotContain("fixture-provider-key", body);
        await client.GetAsync("/api/prizepicks/board?sport=basketball_nba");
        Assert.Equal(2, handler.Requests.Count);
    }
    [Fact]
    ...
    public async Task Does_not_leak_provider_errors_or_repeatedly_spend_quota_after_failure()
    {
        ...
        await feed.ScanAsync("basketball_nba", default); Assert.Single(handler.Requests);
    }
}
